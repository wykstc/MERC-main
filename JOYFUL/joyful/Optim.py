...
class Optim:
    ...
    def set_parameters(self, params, name):
        ...
        if name == "sgd":
            self.optimizer = optim.SGD(
                self.params, lr=self.lr, weight_decay=self.weight_decay,  momentum=0.9
            )
        elif name == "rmsprop":
    ...
    def get_scheduler(self, sch):
        ...
        if sch == "reduceLR":
            sched = lr_scheduler.ReduceLROnPlateau(self.optimizer, "min", min_lr=3e-10, patience=8)
        elif sch == "expLR":
            sched = lr_scheduler.ExponentialLR(self.optimizer, gamma=0.95)
        return sched
